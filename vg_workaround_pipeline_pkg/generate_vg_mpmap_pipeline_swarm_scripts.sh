...
do
    ...
    map_command="module load singularity; mkdir -p /lscratch/\$SLURM_JOB_ID/singularity_cache; export SINGULARITY_CACHEDIR=/lscratch/\$SLURM_JOB_ID/singularity_cache/; export TMPDIR=/lscratch/\$SLURM_JOB_ID; cd ${WORK_DIR}; time singularity -q exec -H ${WORK_DIR}:${HOME} --pwd ${HOME} -B ${GRAPH_FILES_DIR_PATH}:/mnt docker://${VG_CONTAINER} vg mpmap -S -f ${READ_FILE_1_CONTAINER_PATH} -f ${READ_FILE_2_CONTAINER_PATH} -x /mnt/${XG_FILE_NAME} -g /mnt/${GCSA_FILE_NAME} -t ${ALIGNMENT_CORES} > ${SWARM_MAP_OUTPUT_WORKDIR_PATH}/${SAMPLE_NAME}_${i}.gam && rm -f ${WORK_DIR}/${FASTQ_DIR_NAME}/fq_chunk_1.part.${i}.fq.gz ${WORK_DIR}/${FASTQ_DIR_NAME}/fq_chunk_2.part.${i}.fq.gz && time singularity -q exec -H ${WORK_DIR}:${HOME} --pwd ${HOME} -B ${GRAPH_FILES_DIR_PATH}:/mnt docker://${VG_CONTAINER} vg surject -i -x /mnt/${XG_FILE_NAME} -t ${SURJECT_CORES} -b ${HOME}/${GAM_DIR_NAME}/${SAMPLE_NAME}_${i}.gam > ${CHR_OUTPUT_BAM_DIR_PATH}/${SAMPLE_NAME}_${i}.bam && rm -f ${SWARM_MAP_OUTPUT_WORKDIR_PATH}/${SAMPLE_NAME}_${i}.gam"
